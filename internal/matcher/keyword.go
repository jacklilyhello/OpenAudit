	...
	for _, r := range rs {
		for _, kw := range r.Keywords {
			...
			for _, candidate := range variant.TraditionalSimplifiedVariants(kw) {
				...
				added[pattern] = true
				a.Add(pattern, AhoPayload{Type: "keyword", RuleID: r.ID, MatchedRuleName: r.Description, Category: r.Category, RiskLevel: r.RiskLevel, Action: r.Action, Match: kw, NormalizedMatch: pattern, Score: risk.Score(r.RiskLevel, r.Score), Description: r.Description, Source: r.Source, Tags: r.Tags, Provenance: r.Provenance, Behavior: r.Behavior})
			}
		}
	}
	a.Build()
...
func (m KeywordMatcher) Match(text string) []Hit {
	...
	for _, x := range m.aho.Match(text) {
		p := x.Payload
		hits = append(hits, Hit{Type: p.Type, RuleID: p.RuleID, MatchedRuleName: p.MatchedRuleName, Category: p.Category, RiskLevel: p.RiskLevel, Action: p.Action, Match: p.Match, NormalizedMatch: p.NormalizedMatch, SourceText: p.Match, Start: x.Start, End: x.End, Score: p.Score, Description: p.Description, Source: p.Source, Tags: p.Tags, Provenance: p.Provenance, Behavior: p.Behavior})
	}
	return hits
}
