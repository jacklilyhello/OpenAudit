...
func (m MappingMatcher) Match(text string) []Hit {
	...
	for _, x := range m.aho.Match(text) {
		p := x.Payload
		hits = append(hits, Hit{Type: p.Type, VariantType: p.VariantType, RuleID: p.RuleID, MatchedRuleName: p.MatchedRuleName, Category: p.Category, RiskLevel: p.RiskLevel, Action: p.Action, Match: p.Match, NormalizedMatch: p.NormalizedMatch, SourceText: p.SourceText, Canonical: p.Canonical, Variant: p.Variant, Start: x.Start, End: x.End, Score: p.Score, Explanation: p.Explanation, Description: p.Description, Source: p.Source, Tags: p.Tags, Provenance: p.Provenance, Behavior: p.Behavior})
	}
	return hits
}
